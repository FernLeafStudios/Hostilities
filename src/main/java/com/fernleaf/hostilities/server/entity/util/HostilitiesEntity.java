...
public abstract class HostilitiesEntity extends TamableAnimal implements OwnableEntity {
    ...
    protected static final EntityDataAccessor<Boolean> DATA_CHARMED =
            SynchedEntityData.defineId(HostilitiesEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    public int getAnimationId() {
        return this.entityData.get(DATA_ANIMATION_ID);
    }

    public void setAnimationId(int id) {
        this.entityData.set(DATA_ANIMATION_ID, id);
    }
    ...
    // --- Interaction Handling & Taming ---
    @Override
    public @NotNull InteractionResult mobInteract(@NotNull Player player, @NotNull InteractionHand hand) {
        ItemStack stack = player.getItemInHand(hand);

        // 1. Cycle state on empty-hand interact by owner
        if (isTamed() && isOwner(player) && hand == InteractionHand.MAIN_HAND && stack.isEmpty()) {
        ...
        // 2. Taming Logic
        if (this.level().isClientSide) {
            boolean isInteractable = this.isOwner(player) || this.isTamed() || this.isTameItem(stack);
            return isInteractable ? InteractionResult.CONSUME : InteractionResult.PASS;
        } else {
            if (!this.isTamed() && this.isTameItem(stack)) {
    ...
    /**
     * Override in child classes to specify taming items.
     */
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide()) {
            if (this.animationCountdown > 0) {
                ...
                if (this.animationCountdown == 0) {
                    this.setAnimationId(0); // Resets back to ANIM_IDLE
                }
            }
        }
    }
    ...
    public void addAdditionalSaveData(@NotNull CompoundTag tag) {
        ...
        tag.putInt("AnimationCountdown", this.animationCountdown);

        if (this.stationPost != null) {
            tag.put("StationPost", NbtUtils.writeBlockPos(this.stationPost));
        }
    ...
    public void readAdditionalSaveData(@NotNull CompoundTag tag) {
        ...
        if (tag.contains("AnimationCountdown")) {
            this.animationCountdown = tag.getInt("AnimationCountdown");
        }
        if (tag.contains("StationPost")) {
            setStationPost(NbtUtils.readBlockPos(tag, "StationPost").orElse(null));
        }
